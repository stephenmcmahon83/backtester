...
// FIX: Renamed this constant back to its original correct name
...
const MultiSelectDropdown = ({ options, selected, onToggle, maxSelection = 3 }: { options: string[], selected: string[], onToggle: (option: string) => void, maxSelection?: number }) => {
    ...
    const handleToggle = (option: string) => {
        if (!selected.includes(option) && selected.length >= maxSelection) {
            alert(`You can select a maximum of ${maxSelection} tickers to prevent server timeout.`);
            return;
        }
        onToggle(option);
    };
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button onClick={() => setIsOpen(!isOpen)} className="w-full px-4 py-2 border border-gray-300 rounded-md bg-white text-left overflow-hidden whitespace-nowrap text-ellipsis">
                {selected.length > 0 ? selected.join(', ') : 'Select Tickers...'}
            </button>
            {isOpen && (
                <div className="absolute z-20 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-96 overflow-y-auto">
                    <input type="text" placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full px-4 py-2 border-b sticky top-0" />
                    <ul>
                        {filteredOptions.map(option => (
                            <li key={option} className="px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center" onClick={() => handleToggle(option)}>
                                <input type="checkbox" checked={selected.includes(option)} readOnly className="mr-2" />
                                {option}
                            </li>
                        ))}
                    </ul>
...
export default function PortfolioTrendBacktesterPage() {
  ...
  const handleRunBacktest = async () => {
    if (selectedTickers.length === 0) { setError("Please select at least one ticker."); return; }
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-8 font-sans text-gray-900">
      <main className="container mx-auto max-w-7xl bg-white p-6 rounded-lg shadow-xl">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-extrabold text-gray-900">Portfolio Trend Backtester</h1>
          <p className="text-gray-500 mt-2">Each ticker starts with $10k. Profits are compounded and reinvested into the ticker they came from.</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-center items-end gap-4 mb-8 p-6 bg-slate-50 rounded-xl border border-slate-200 shadow-sm">
          <div className="w-full md:w-1/4">
            <label className="block text-sm font-bold text-gray-700 mb-1">Portfolio (Max 3)</label>
            {loadingTickers ? <div className="h-10 w-full bg-gray-200 rounded animate-pulse"></div> : (
              <MultiSelectDropdown options={tickerList} selected={selectedTickers} onToggle={handleTickerToggle} maxSelection={3} />
            )}
          </div>
          ...
          <div className="w-full md:w-auto self-end">
            <button onClick={handleRunBacktest} disabled={loading || loadingTickers || selectedTickers.length === 0}
        ...
        {results && (
          <div className="space-y-12 animate-fade-in">
            <div className="p-4 border border-gray-200 rounded-lg bg-white shadow-md">
              <BacktestChart data={results} title={`Portfolio Performance - ${strategy.replace(/_/g, ' ').toUpperCase()}`} />
            </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                           <thead className="bg-gray-50"><tr><th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Overall</th><th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Result</th></tr></thead>
                            <tbody className="bg-white divide-y divide-gray-200 text-sm">
                                ...
                                <tr><td className="px-4 py-3 font-medium text-gray-900">Current Position</td><td className={`px-4 py-3 text-right font-bold ${results.isHolding ? 'text-blue-600' : 'text-gray-400'}`}>{results.isHolding ? "IN POSITIONS" : "CASH"}</td></tr>
