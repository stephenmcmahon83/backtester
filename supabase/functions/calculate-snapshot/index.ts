...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. CALCULATE STATS FOR ALL TICKERS INDIVIDUALLY
    const snapshotResults = await Promise.all(
      uniqueTickers.map(async (ticker) => {
        ...
        if (!history || history.length < 31) return null;
        ...
        let sumTrailingRet1 = 0, sumTrailingRet5 = 0, sumTrailingRet10 = 0;
        ...
        for (let day = 1; day <= 20; day++) {
            ...
            instances.forEach((idx) => {
                ...
                const endPrice = history[idx + day].close;
